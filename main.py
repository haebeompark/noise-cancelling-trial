from vecorization import Vecto
'''
Copyright (C)2020 KAURML  <ingeechart@kau.kr>

학교 머신러닝수업의 로지스틱 회귀 과제로부터 계속 발전시켜온 코드입니다.
...
class NeuralNetwork:
    ...
    #     Y = cache["Y"]
    #     grads = {}
    #     dims = int(len(parameters)/2) #(=3)
    #     A = cache["A"+str(dims)] #A3
    #     dZ = A-Y #dZ3 = A3 - Y
    #     while(dims>0): #dims = 3,2,1
    #         if(dims > 0):
    #             W = parameters["W"+str(dims)] #W3, W2, W1
    #             A = cache["A"+str(dims-1)] #A2, A1, A0
    #             dW = (1/self.nSample) * np.dot(dZ,A.T) #dW3 = (1/m)*(dZ3 dot A1.T), dW2, dW1
    #             db = (1/self.nSample) * np.sum(dZ,axis=1,keepdims=True) #db3 = (1/m)*sum(dZ3), db2, db1
    ...
    #     dWj1, dWj2, dWj3, dWj4, dWj5 = self.grads["dW1"], self.grads["dW2"], self.grads["dW3"], self.grads["dW4"], self.grads["dW5"]

    #     dW1 = dWj1 + (lambd/self.nSample)*(np.sum(W1))
    #     dW2 = dWj2 + (lambd/self.nSample)*(np.sum(W2))
    #     dW3 = dWj3 + (lambd/self.nSample)*(np.sum(W3))
    #     dW4 = dWj4 + (lambd/self.nSample)*(np.sum(W4))
    #     dW5 = dWj5 + (lambd/self.nSample)*(np.sum(W5))

    #     self.grads.update(dW1=dW1,dW2=dW2,dW3=dW3,dW4=dW4,dW5=dW5)
...
def training_with_regularization(X_train, Y_train, simpleNN, num_iterations, learning_rate):
    ...
    for i in range(0, num_iterations):
        AL = simpleNN.forward(X_train)               ##forward propagation 하기.
        cost = simpleNN.compute_cost_with_regularization(AL,Y_train)             ## cost function을 이용해 cost 구하기.
...
def main():
    ...
    ### dataset loading 하기.
    X_train = np.random.rand(1,20000)
    Y_train = Vecto.vectorization(np.sin(X_train * np.pi),count)
    X_train = Vecto.vectorization(X_train,count)
    X_test = np.random.rand(1,20)
    Y_test = Vecto.vectorization(np.sin(X_test * np.pi),count)
    X_test = Vecto.vectorization(X_test,count)
    # plt.title("Data distribution")
    # plt.scatter(X_train[0, :], X_train[1, :], c=Y_train[0,:], s=20, cmap=plt.cm.RdBu)
    # plt.show()
    
    ## 코딩시작
    layerDims = [count + 1,32,64,64,32,count + 1]
    ...
    ### training
    ## 코딩 시작
    training_with_regularization(X_train,Y_train, simpleNN, num_iterations, learning_rate)
    '''
    for i in range(0, num_iterations):
        A3 = simpleNN.forward(X_train)               ##forward propagation 하기.
        cost = simpleNN.compute_cost(A3,Y_train)             ## cost function을 이용해 cost 구하기.
        pass                ## 위에 만든 함수를 이용하여 backpropagation 진행
        simpleNN.backward()
        pass                ## 위에 만든 함수를 이용하여 network의 weight update하기.
        simpleNN.update_params(lr)
        if i % 1000 ==0:
            lr = lr * 0.99
            print("Cost after iteration %i: %f" %(i,cost))
    '''
    ...
    print("실제 값\n",np.sin(X_test))
    print("\n\n",predictions)
    unvec = unvectorization(predictions, count) #이게 도출
    print("\n\n정답vector\n",Y_test)
    print("\n\n모듈이 구한 답\n",unvec)
    print("\n\n정답을 unvec ")
    print(unvectorization(Y_test,count))
    print("\n\n오차비율 ")
    error = ((unvec - unvectorization(Y_test,count)) / (unvec + unvectorization(Y_test,count))*2)
